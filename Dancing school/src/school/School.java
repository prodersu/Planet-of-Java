...
public class School extends JFrame {
    ...
    public School(String s){
        ...
        enr_toCourse = new EnrollToCourse(this);
        clist = new CourseList(this);
        look_for = new LookFor(this);
        
        add(signIn);add(admin);add(admin_add);add(getcourse);add(rem_course);add(income); add(edit);
        add(admin_client);add(client);add(enr_toCourse);add(clist);add(look_for);
    ...
    public void checkDiary(){
        ...
        {
            String diary = "You have " + (Diary.size() + 1) + " courses in your Diary. \n";
            int s = 0;
